...
function AuthorsList() {
  ...
  const toggleAuthorStatus = async (authorObj) => {
    const nextStatus = !authorObj.isAuthorActive;
    try {
      const res = await api.patch(
        `/admin-api/authors/${authorObj._id}/status`,
        { isAuthorActive: nextStatus },
      );

      if (res.status === 200) {
        setAuthors((prev) =>
          prev.map((author) => (author._id === authorObj._id ? { ...author, isAuthorActive: nextStatus } : author)),
        );
      }
    } catch (err) {
      ...
      <div className={articleGrid}>
        {authors.map((author) => (
          <div key={author._id} className={`${articleCardClass} rounded-2xl`}>
            ...
            <p className="text-xs text-[#a1a1a6] mt-3">Status: {author.isAuthorActive ? "Active" : "Blocked"}</p>
            ...
            >
              {author.isAuthorActive ? "Block" : "Unblock"}
            </button>
          </div>
        ))}
      </div>
